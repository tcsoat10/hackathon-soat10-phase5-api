...
output "mongo_uri" {
  value = "mongodb://${var.mongo_user}:${var.mongo_password}@${helm_release.main-api-db.name}:27017/${var.db_name}?authSource=${var.db_name}"
}
...
output "mongo_host" {
  ...
  value       = helm_release.main-api-db.name
}
